...
the overlay. Note `auto_pass_turn` now also carries a hotkey (`auto_pass_turn_hotkey`), but that key
*toggles* the switch rather than enabling the feature by being set: it is the one hotkey where a blank
does not turn anything off, only leaves the switch without a key. The boolean stays the source of truth,
and stays session-only; only the key combination is persisted, like every other keybind.
...
and sex pinned to each) and the eight
